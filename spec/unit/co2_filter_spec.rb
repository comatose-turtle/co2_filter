...
describe Co2Filter do
  ...
  context '#filter' do
    ...
    it 'should not call the content-based filter if content-based results are provided' do
      allow(Co2Filter::ContentBased).to receive(:filter)
      result = Co2Filter::ContentBased::Results.new({})
      Co2Filter.filter(current_user: user1, other_users: other_users, content_based_results: result)
      expect(Co2Filter::ContentBased).not_to have_received(:filter)
    end

    it 'should throw an error if content-based results and items are nil' do
      ...
      expect{Co2Filter.filter(current_user: user1, other_users: other_users, content_based_results: nil, items: nil)}.to raise_error(ArgumentError)
    end
